...
public class FriendController {
    ...
    public ResponseEntity<List<Friend>> getFollowers(@PathVariable("id") Integer userId) {
        return new ResponseEntity<List<Friend>>(friendService.getFollowers(userId), HttpStatus.OK);
    }

    @GetMapping({"/following/{id}"})
    public ResponseEntity<List<Friend>> getFollowing(@PathVariable("id") Integer userId){
        return new ResponseEntity<List<Friend>>(friendService.getFollowing(userId), HttpStatus.OK);
    }

    @PostMapping
    public ResponseEntity<Friend> follow(@RequestBody Friend friend) {
        return new ResponseEntity<Friend>(friendService.follow(friend), HttpStatus.CREATED);
    }

    @DeleteMapping("/followers/{id1}/{id2}")
    @Transactional
    public ResponseEntity<String> removeFollower(@PathVariable("id1") Integer user1, @PathVariable("id2") Integer user2) {
        return new ResponseEntity<String>(friendService.removeFollower(user1, user2), HttpStatus.OK);
    }

    @DeleteMapping("/following/{id1}/{id2}")
    @Transactional
    public ResponseEntity<String> stopFollowing(@PathVariable("id1") Integer user1, @PathVariable("id2") Integer user2) {
